...
$sql = "SELECT * FROM admin.qol_labreservelist WHERE (reservedate = '$day' and purpose>0) and ((starttime<=$end_time) and (endtime>=$start_time));";
...
if ($result->num_rows > 0 && $purpose!=0) {
    ...
    $sql = "INSERT INTO admin.qol_labreservelist(purpose, studentname, reservedate, starttime, endtime, groupsize, password) VALUES('$purpose', '$reservename', '$day', $start_time, $end_time, '$groupsize', '$password')";
    //insertion query
    $result = $conn->query($sql);
    if($result === TRUE){
        $response = "예약 성공!";
